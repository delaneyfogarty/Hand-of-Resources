...
module.exports = class Cat {
  ...
  static async updateCat(id, attrs) {
    const cat = await Cat.getSingleCat(id);
    if (!cat) return null;
    const { name, color, favorite_food, age } = { ...cat, ...attrs };
    const { rows } = await pool.query(
      `UPDATE cats
        SET name=$2, color=$3, favorite_food=$4, age=$5
        WHERE id=$1 RETURNING *`,
      [id, name, color, favorite_food, age]
    );
    return new Cat(rows[0]);
  }
};
